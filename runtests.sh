...
cd $BUGZILLA_HOME
git stash
git checkout $BUGZILLA_REPO_BRANCH
/usr/bin/git pull --rebase
if [ "$BUGZILLA_REPO_COMMIT" != "" ]; then
    git checkout $BUGZILLA_REPO_COMMIT
fi
...
cd $BUGZILLA_HOME
/usr/bin/git clone $BUGZILLA_QA_REPO -b $BUGZILLA_REPO_BRANCH qa
...
if [ "$TEST_SUITE" = "selenium" ]; then
    ...
    prove -f -I$BUGZILLA_HOME/lib test_*.t
    exit $?
fi
...
if [ "$TEST_SUITE" = "webservices" ]; then
    ...
    cd $BUGZILLA_HOME/qa/t
    prove -f -I$BUGZILLA_HOME/lib webservice_*.t
    exit $?
fi
